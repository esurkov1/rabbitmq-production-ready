...
class RabbitMQClient extends EventEmitter {
  ...
  _validateConfig(connectionString, options) {
    ...
    // Валидация serializer/deserializer
    if (options.serializer !== undefined && typeof options.serializer !== 'function') {
      throw new Error('serializer must be a function');
    }
    if (options.deserializer !== undefined && typeof options.deserializer !== 'function') {
      throw new Error('deserializer must be a function');
    }

    // Валидация tracing конфига
    if (options.tracing) {
      if (options.tracing.enabled !== undefined && typeof options.tracing.enabled !== 'boolean') {
        throw new Error('tracing.enabled must be a boolean');
      }
      if (
        options.tracing.headerName !== undefined &&
        (typeof options.tracing.headerName !== 'string' || !options.tracing.headerName)
      ) {
        throw new Error('tracing.headerName must be a non-empty string');
      }
      if (
        options.tracing.correlationIdHeader !== undefined &&
        (typeof options.tracing.correlationIdHeader !== 'string' ||
          !options.tracing.correlationIdHeader)
      ) {
        throw new Error('tracing.correlationIdHeader must be a non-empty string');
      }
      if (
        options.tracing.getTraceContext !== undefined &&
        typeof options.tracing.getTraceContext !== 'function'
      ) {
        throw new Error('tracing.getTraceContext must be a function');
      }
      if (
        options.tracing.setTraceContext !== undefined &&
        typeof options.tracing.setTraceContext !== 'function'
      ) {
        throw new Error('tracing.setTraceContext must be a function');
      }
      if (
        options.tracing.generateTraceId !== undefined &&
        typeof options.tracing.generateTraceId !== 'function'
      ) {
        throw new Error('tracing.generateTraceId must be a function');
      }
    }
  }
  ...
  _getCorrelationId(msg, options) {
    // Если correlation ID явно передан в options, используем его
    if (options?.correlationId) {
      return options.correlationId;
    }

    // Если есть сообщение, пытаемся получить correlation ID из заголовков
    if (msg?.properties?.headers) {
      const headerName = this.tracing.correlationIdHeader;
    ...
    // Генерируем новый correlation ID автоматически
    return this._generateCorrelationId();
  }

  /**
   * Получить trace ID из контекста или сгенерировать новый
   * @private
   */
  _getTraceId(options) {
    // Если trace ID явно передан в options, используем его
    if (options?.traceId) {
      return options.traceId;
    }

    // Если tracing включен, пытаемся получить из контекста
    if (this.tracing.enabled) {
      ...
      // Если trace ID не найден в контексте, генерируем новый автоматически
  ...
  /**
   * Установить trace context из заголовков сообщения
   * @private
   */
  _setTraceContextFromMessage(msg) {
    ...
    const traceId = headers[this.tracing.headerName];
    const correlationId = headers[this.tracing.correlationIdHeader];
    ...
    // Также устанавливаем correlation ID если нужно
    if (correlationId && this.tracing.setTraceContext) {
      // Можно расширить логику если нужно устанавливать correlation ID отдельно
    }
  }
  ...
  _registerShutdownHandlers() {
    ...
    // Увеличиваем лимит слушателей для process, чтобы избежать предупреждений
    // при создании множества клиентов в тестах
   ...
   */
  isConnected() {
    // Проверяем что соединение и канал существуют
    if (!this.connection || !this.channel) {
      return false;
    }

    try {
      // Проверяем состояние сокета соединения
      const socket = this.connection.connection;
      if (!socket || socket.destroyed) {
        return false;
      }

      // Проверяем readyState - может быть 'open', 'opening', 'readOnly', 'writeOnly'
      // Считаем соединение активным если оно не закрыто
  ...
  async publish(queue, message, options = {}) {
    ...
    try {
      // Используем serializer для преобразования сообщения
      const messageBuffer = this.serializer(message);

      // Формируем заголовки
      const headers = {
        ...options.headers,
        [this.tracing.correlationIdHeader]: correlationId,
      };

      // Добавляем trace ID если включен tracing
      // Если trace ID уже есть в заголовках, используем его, иначе добавляем автоматически сгенерированный
      if (this.tracing.enabled) {
        if (!headers[this.tracing.headerName] && traceId) {
          headers[this.tracing.headerName] = traceId;
        }
      }
  ...
  async consume(queue, handler, options = {}) {
    ...
    const result = await this.channel.consume(
      ...
      async (msg) => {
        ...
        // Устанавливаем trace context из заголовков сообщения
        ...
        try {
          ...
          // Десериализуем сообщение если нужно
          // Создаем обертку для msg с десериализованным контентом
          const deserializedContent = this.deserializer(msg.content);
          const enhancedMsg = {
            ...msg,
            content: msg.content, // Оригинальный Buffer
            parsedContent: deserializedContent, // Десериализованный контент
          };
