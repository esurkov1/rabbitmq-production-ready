...
class RabbitMQClient extends EventEmitter {
  constructor(connectionString, options = {}) {
    ...
    this.correlationIdGenerator = options.correlationIdGenerator || (() => {
  ...
  _validateConfig(connectionString, options) {
    ...
    if (options.reconnectMultiplier !== undefined && 
  ...
  _validateRetryConfig(retryConfig, configName) {
    ...
    if (retryConfig.multiplier !== undefined && 
  ...
  async publish(queue, message, options = {}) {
    ...
    try {
      ...
      if (this.hooks.onPublish) {
        try {
          this.hooks.onPublish({ queue, exchange: '', messageSize: messageBuffer.length, duration, correlationId });
        } catch (hookError) {
  ...
  async publishToExchange(exchange, routingKey, message, options = {}) {
    ...
    try {
      ...
      if (this.hooks.onPublish) {
        try {
          this.hooks.onPublish({ queue: '', exchange, routingKey, messageSize: messageBuffer.length, duration, correlationId });
        } catch (hookError) {
          this.logger.warn({ err: hookError }, 'Error in onPublish hook');
        }
      }
      ...
      if (this.hooks.onError) {
        try {
          this.hooks.onError({ type: 'publish', error, queue: '', exchange, routingKey, correlationId });
        } catch (hookError) {
          this.logger.warn({ err: hookError }, 'Error in onError hook');
        }
      }
  ...
  async consume(queue, handler, options = {}) {
    ...
    const result = await this.channel.consume(queue, async (msg) => {
      ...
      const maxRetries = options.maxRetries !== undefined 
        ? options.maxRetries 
        : (this.consumeRetry.enabled ? this.consumeRetry.maxAttempts : 0);
      const useRetry = options.retry !== false && this.consumeRetry.enabled && retryCount < maxRetries;
      ...
      try {
        ...
        if (this.hooks.onConsume) {
          try {
            this.hooks.onConsume({ queue, processingTime: processingTimeSeconds, correlationId, retryCount });
          } catch (hookError) {
            this.logger.warn({ err: hookError }, 'Error in onConsume hook');
          }
        }
        
        if (this.metrics.consume.minProcessingTime === null || processingTime < this.metrics.consume.minProcessingTime) {
          this.metrics.consume.minProcessingTime = processingTime;
        }
        if (this.metrics.consume.maxProcessingTime === null || processingTime > this.metrics.consume.maxProcessingTime) {
  ...
  getMetrics() {
    ...
    const avgProcessingTime = this.metrics.consume.totalConsumed > 0
    ...
    return {
      connection: {
        ...
        uptime: this.metrics.connection.lastConnectionTime && this.metrics.connection.lastDisconnectionTime
          ? this.metrics.connection.lastDisconnectionTime - this.metrics.connection.lastConnectionTime
      ...
      publish: {
        ...
        averageMessageSize: this.metrics.publish.totalPublished > 0
      ...
      consume: {
        ...
        errorRate: this.metrics.consume.totalConsumed > 0
          ? this.metrics.consume.consumeErrors / (this.metrics.consume.totalConsumed + this.metrics.consume.consumeErrors)
          : 0
      },
      queue: {
        ...this.metrics.queue
