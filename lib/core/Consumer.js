...
class Consumer {
  ...
  async _processMessage(queue, msg, handler, options) {
    ...
    const maxRetries = this._getMaxRetries(options);
  ...
  async _handleConsumeError(queue, msg, error, options, retryCount, maxRetries) {
    ...
    if (this.options.consumeRetry.enabled && options.retry !== false && retryCount < maxRetries) {
  ...
  _shouldUseRetry(options, retryCount, maxRetries) {
    return options.retry !== false && this.options.consumeRetry.enabled && retryCount < maxRetries;
  }
