...
class ConnectionManager extends EventEmitter {
  ...
  async connect() {
    ...
    try {
      ...
      // Если autoReconnect включен, не пробрасываем ошибку - реконнект уже запланирован
      if (this.options.autoReconnect && this._isRabbitMQError(error)) {
        this.logger.warn({ err: error }, 'Initial connection failed, reconnect scheduled');
        // Не бросаем ошибку - реконнект уже запланирован в _connect()
        return;
      }
    ...
    try {
      ...
      if (connectionError) {
        ...
        if (this.options.autoReconnect && !this.isShuttingDown) {
          ...
          // Не бросаем ошибку - reconnect запланирован
          return;
        }
