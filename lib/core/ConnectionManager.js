...
class ConnectionManager extends EventEmitter {
  ...
  async connect() {
    ...
    try {
      ...
      return this._waitForConnection({ scheduleReconnect: true });
    }
  }
  ...
  async _connect() {
    try {
      ...
      if (connectionError) {
        this.connection = null;
        this.channel = null;
        this._handleReconnectableError(connectionError);
      ...
      if (channelError) {
        this.connection = null;
        this.channel = null;
        this._handleReconnectableError(channelError);
        throw channelError;
      }
      ...
      if (this._isHeartbeatTimeoutError(error)) {
        this._handleReconnectableError(error, {
          logMessage: 'Heartbeat timeout during connect, will reconnect',
          logLevel: 'warn',
        });
      } else if (this._isRabbitMQError(error)) {
        this._handleReconnectableError(error, {
          logMessage: 'Failed to connect to RabbitMQ',
          logLevel: 'error',
        });
      } else {
  ...
  _isRabbitMQError(error) {
    ...
    const errorCandidates = [error];
    if (error.cause && typeof error.cause === 'object') {
      errorCandidates.push(error.cause);
    }
    if (Array.isArray(error.errors)) {
      errorCandidates.push(...error.errors);
    }
    ...
    const amqplibPatterns = ['amqplib', 'node_modules/amqplib', 'connection.js', 'channel.js'];
    ...
    return errorCandidates.some((candidate) => {
      ...
      const message = candidate.message || candidate.toString() || '';
      ...
      const codeMatch = rabbitMQErrorPatterns.some((pattern) =>
        code.toString().toLowerCase().includes(pattern.toLowerCase())
      );

      return messageMatch || stackMatch || codeMatch;
    });
  }
  ...
  _handleHeartbeatTimeout(error) {
    ...
    this._scheduleReconnectIfNeeded();
  }
  ...
  _registerUncaughtExceptionHandler() {
    ...
    const existingHandler = process
      .listeners('uncaughtException')
      .find((handler) => handler._isRabbitMQHandler);
  ...
  /**
   * Планирование переподключения (если можно)
   */
   ...
   */
  _handleReconnectableError(error, { logMessage, logLevel = 'warn', emitError = true } = {}) {
    if (logMessage) {
      const loggerFn = typeof this.logger[logLevel] === 'function' ? logLevel : 'warn';
      this.logger[loggerFn]({ err: error }, logMessage);
    }
   ...
   */
  _waitForConnection({ timeout = null, scheduleReconnect = false } = {}) {
    if (
  ...
  async waitForConnection(timeout = 30000) {
    ...
    return this._waitForConnection({ timeout });
  }
